...
export default function DownloadTab({ onStartDownload, settings }) {
  const [form, setForm] = useState(DEFAULT_FORM);

  // Apply saved defaults from Settings tab when they load
  useEffect(() => {
    if (settings?.defaults) {
      setForm((prev) => ({ ...prev, ...settings.defaults }));
    }
  }, [settings?.defaults]);
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4 space-y-1">
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5 leading-snug">
              {form.komikku ? (
                <>
                  ...
                  <span className="font-mono">mangas/&lt;Series&gt;/</span>.
                </>
              ) : (
                <>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-3">
          <div>
            ...
            <Input id="chapters" value={form.chapters} onChange={(e) => set("chapters", e.target.value)} placeholder="all, or 1-50, 75, 80-100" className="mt-1" />
          </div>
          <div>
